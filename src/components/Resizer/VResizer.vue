...
import ResizeIcon from '../Icon/ResizeIcon.vue'
import ResizeHorizontalIcon from '../Icon/ResizeHorizontalIcon.vue';
import ResizeVerticalIcon from '../Icon/ResizeVerticalIcon.vue';
...
export default defineComponent({
    ...
    components: { ResizeIcon, ResizeHorizontalIcon, ResizeVerticalIcon }
})
</script>
  ...
  <div ref="root" :class="`${className} ${resize}`">
    ...
    <slot name="handle" v-bind="{ startDrag, resize }">
      <div ref="handle" class="wi-resize-handle" :class="resize" @mousedown="startDrag">
        <ResizeIcon v-if="resize === 'both'" />
        <ResizeHorizontalIcon v-else-if="resize === 'height'" />
        <ResizeVerticalIcon v-else />
      </div>
    </slot>
  </div>
</template>
